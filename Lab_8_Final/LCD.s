...
STEP1
	LDR R1, =SSI0_SR_R
	LDR R1, [R1]
	AND R1, #0x10 ;clear everything but bit 4
	CMP R1, #0x10 
	BEQ STEP1 ;loop back to step 1
	LDR R1, =DC
	MOV R2, #0x00
	STR R2, [R1] ;clear PA6 to zero
	LDR R1, =SSI0_DR_R
	STR R0, [R1] ;write command in R0 to SSIO_DR_R
STEP5
	LDR R1, =SSI0_SR_R
	LDR R1, [R1]
	AND R1, #0x10 ;check bit 4 of SSIO_SR_R
	CMP R1, #0x10
	BEQ STEP5
...
STEP_1
	...
	STR R2, [R1] ;PA6 = 1
	LDR R1, =SSI0_DR_R
	STR R0, [R1] ;write r0 to SSI0_DR_R   
    POP {R1-R11, PC}     ;   return
