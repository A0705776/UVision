...
int main(void){      
  ...
  while(1){ 
		switch(Piano_In()){ 
			case 1: Sound_Play(C); break; 	//PE0
			case 2: Sound_Play(B); break; 	//PE1
			case 3: Sound_Play(BF); break;	//PE0 + PE1
			case 4: Sound_Play(A); break; 	//PE2
			case 5: Sound_Play(AF); break;	//PE2 + PE0
			case 6: Sound_Play(G); break; 	//PE2 + PE1
			case 7: Sound_Play(GF); break;	//PE2 + PE1 + PE0
			default: break;
		}
		heartBeat();
  } 
